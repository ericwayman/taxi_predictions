...
class PreProcessData:
    ...
    def __init__(self, train_path,test_path,target):
        ...
        self.train_df = preprocess_train_data(train_df)
        self.CountFeatures = CF.CountFeatures(in_feat=list(train_df["begin_labels"]),
                                            out_feat=list(test_df["begintrip_lat"]))
        self.X_train, self.y_train=train_data(train_df)

        #preprocess test data
        test_df = pd.read_csv(test_path)
        self.test_df = preprocess_test_data(test_df)

        self.X_test, self.y_test=test_data(test_df)
    ...
    def train_data(train_df):
        X_train = self.CountFeatures.make_features_from_new_data(in_data=train_df["begin_labels"])
        y_train=df[self.target].as_matrix()
        return X_train, y_train

    def test_data(test_df):
        X_train = self.CountFeatures.make_features_from_new_data(in_data=test_df["begin_labels"])
